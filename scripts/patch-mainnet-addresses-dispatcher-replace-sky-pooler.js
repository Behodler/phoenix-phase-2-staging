#!/usr/bin/env node
/**
 * patch-mainnet-addresses-dispatcher-replace-sky-pooler.js
 *
 * Story 056. After broadcasting DispatcherReplaceSkyPoolerAtIndex4.s.sol, this
 * script patches mainnet-addresses.ts with the new BalancerPoolerV2 and
 * BalancerPoolerMintDebtHook addresses pulled from
 * `broadcast/DispatcherReplaceSkyPoolerAtIndex4.s.sol/1/run-latest.json`.
 *
 * Patches in this run:
 *
 *   1. nftsV2.BalancerPooler           expected old = LIVE_INDEX4_POOLER (nudge pooler)
 *                                      patched to  = newPooler from broadcast
 *   2. BalancerPoolerMintDebtHook      expected old = current live hook
 *                                      patched to  = newHook from broadcast
 *
 * Self-validating: refuses to patch unless the current on-file address matches the
 * expected-OLD constant (guards against double-runs / drift).
 *
 * Exit codes:
 *   0 - Success
...
const BROADCAST_FILE = path.join(
    ROOT, 'broadcast', 'DispatcherReplaceSkyPoolerAtIndex4.s.sol', '1', 'run-latest.json'
);
const ADDRESSES_FILE = path.join(ROOT, 'server', 'deployments', 'mainnet-addresses.ts');

// Must match constants in script/DispatcherReplaceSkyPoolerAtIndex4.s.sol and the
// pre-broadcast state of mainnet-addresses.ts.
//   LIVE_INDEX4_POOLER  = 0x26f89f4b46eb164303985795ee20b15bb1edb38a (current nudge pooler)
//   live mint-debt hook = 0x14277a0bc38a6bb7eb732c3a49190030632f727e
const OLD_BALANCER_POOLER_NFTS_V2 = '0x26f89f4b46eb164303985795ee20b15bb1edb38a';
const OLD_BALANCER_POOLER_MINT_DEBT_HOOK = '0x14277a0bc38a6bb7eb732c3a49190030632f727e';
...
/**
 * Walk transactions[], filter transactionType === "CREATE", match deployments by
 * contractName. The cutover broadcast deploys exactly TWO contracts:
 *   1. BalancerPoolerV2   (step 4)
 *   2. BalancerPoolerMintDebtHook (step 5)
 */
...
function refreshHeaderComment(source) {
    ...
    const stampLine = `// Updated ${today}: index-4 dispatcher cut over to Sky-PSM BalancerPoolerV2 + new MintDebtHook (story 056)`;
...
function run() {
    ...
    console.log('=================================================');
    console.log('  patch-mainnet-addresses-dispatcher-replace-sky-pooler');
    console.log('=================================================');
