...
contract DolaStrategyWithdrawalStatus is DolaStrategyWithdrawalBase {
    function run() external view {
        ...
        if (status == STATUS_INITIATED || status == STATUS_EXECUTABLE) {
            ...
            if (nowTs < executableAt) {
                console.log("effective phase:     WAITING - seconds until executable:", executableAt - nowTs);
                console.log("                     seconds until expiry:", expiresAt - nowTs);
            } else if (nowTs <= expiresAt) {
                console.log("effective phase:     EXECUTABLE - cutover may broadcast now; seconds until expiry:", expiresAt - nowTs);
            } else {
                console.log("effective phase:     expired (lazy) - rerun initiate-dola-ys-withdrawal:broadcast");
            }
        } else if (status == STATUS_EXPIRED) {
            console.log("effective phase:     expired - rerun initiate-dola-ys-withdrawal:broadcast");
        } else {
            console.log("effective phase:     none pending (not initiated, or already executed)");
        }
