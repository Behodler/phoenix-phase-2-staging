...
contract MintPageViewTest is Test {
    ...
    function testGetNamesReturnsCorrectFieldNames() public view {
        ...
        // USDS fields
        assertEq(names[18], "USDS-allowance");
        assertEq(names[19], "USDS-price");
        assertEq(names[20], "USDS-growthBasisPoints");
        assertEq(names[21], "USDS-balance");
        assertEq(names[22], "USDS-nftBalance");
        assertEq(names[23], "USDS-dispatcherIndex");
    ...
    function testGetNames_USDSFieldsNamedCorrectly() public view {
        string[] memory names = view_.getNames();
        bytes memory prefix = bytes("USDS-");
        for (uint256 i = 18; i <= 23; i++) {
            bytes memory nameBytes = bytes(names[i]);
            assertGe(nameBytes.length, prefix.length, "name too short");
            for (uint256 j = 0; j < prefix.length; j++) {
                assertEq(nameBytes[j], prefix[j], "name does not start with USDS-");
            }
        }
    }
